        ...
        <Card>
          <CardContent className="pt-6 text-center">
            ...
            <p className="text-sm text-gray-600 mb-6">
              Akun buyer berhasil didaftarkan. Tim admin akan meninjau akun Anda terlebih dahulu.
            </p>
            <Link to="/login">
    ...
    <PublicLayout>
      <Card>
        <CardContent className="pt-6">
          <div className="mb-6">
            <h2 className="text-2xl font-bold text-gray-900">Daftar sebagai Buyer</h2>
            <p className="text-sm text-gray-500 mt-1">Buat akun untuk mulai membuat dan mengelola tender.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <FormField label="Nama Perusahaan" htmlFor="companyName" error={errors.companyName} required>
              <Input id="companyName" placeholder="PT Contoh Indonesia" value={form.companyName} onChange={set('companyName')} error={!!errors.companyName} />
            </FormField>
            <FormField label="Nama Legal" htmlFor="legalName" error={errors.legalName}>
              <Input id="legalName" placeholder="PT Contoh Indonesia Tbk" value={form.legalName} onChange={set('legalName')} />
            </FormField>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <FormField label="Email Perusahaan" htmlFor="companyEmail" error={errors.email}>
                <Input id="companyEmail" type="email" placeholder="info@perusahaan.com" value={form.email} onChange={set('email')} error={!!errors.email} />
              </FormField>
              <FormField label="Telepon" htmlFor="phone" error={errors.phone}>
                <Input id="phone" placeholder="+62 21 xxxxxx" value={form.phone} onChange={set('phone')} />
              </FormField>
            </div>
            <FormField label="Alamat" htmlFor="address" error={errors.address}>
              <Input id="address" placeholder="Jl. Contoh No. 1, Jakarta" value={form.address} onChange={set('address')} />
            </FormField>
            ...
            <FormField label="Nama Pemilik" htmlFor="ownerName" error={errors.ownerName} required>
              <Input id="ownerName" placeholder="Budi Santoso" value={form.ownerName} onChange={set('ownerName')} error={!!errors.ownerName} />
            </FormField>
            <FormField label="Email Pemilik" htmlFor="ownerEmail" error={errors.ownerEmail} required>
              <Input id="ownerEmail" type="email" placeholder="budi@perusahaan.com" value={form.ownerEmail} onChange={set('ownerEmail')} error={!!errors.ownerEmail} />
            </FormField>
